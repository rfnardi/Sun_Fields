...
\begin{figure}[!htb]
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			\addplot[mark size=0.5pt, color=red] table[x=array_size, y=power, col sep=semicolon] {../../data/pow_dep_array_size.dat};
		\end{axis}
	\end{tikzpicture}
...
\begin{figure}[!htb]
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			xlabel={x}, 
			ylabel={Power ($W$)}, 
			title={Incremneto de potência refletida por uma malha situada ao longo da direção leste-oeste}]
			\addplot[mark size=0.5pt, color=blue] table[x=array_size, y=delta, col sep=semicolon] {../../data/pow_dep_array_size.dat};
		\end{axis}
	\end{tikzpicture}
